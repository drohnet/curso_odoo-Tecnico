    ...
    def _check_academy_name(self, cr, uid, ids, context=None):
        ...
        if current_obj.is_academy == True:
            ...
            if previous_name_ids:
                return False
        return True
    def _check_teacher_name_surname(self,cr,uid,ids,context=None):
        if context is None:
            context= {}
        teacher_obj=self.browse(cr,uid,ids,context=context)
        _logger.info(teacher_obj)
        _logger.info(teacher_obj.is_teacher)
        if teacher_obj.is_teacher == True :
            _logger.info('*******************is_teacher****************')
            teacher_obj_search= self.search(cr,uid,[('id','!=',teacher_obj.id),('name','=',teacher_obj.name),('surname','=',teacher_obj.surname),('is_teacher','=',True)],context=context)
            if teacher_obj_search :
                return False
        return True
    def _check_student_name_surname(self,cr,uid,ids,context=None):
        if context is None:
            context= {}
        student_obj=self.browse(cr,uid,ids,context=context)
        _logger.info(student_obj)
        _logger.info(student_obj.is_student)
        if student_obj.is_student == True :
            _logger.info('****************is_student*********************')
            teacher_obj_search= self.search(cr,uid,[('id','!=',student_obj.id),('name','=',student_obj.name),('surname','=',student_obj.surname),('is_student','=',True)],context=context)
            if teacher_obj_search :
                return False
        return True
    _columns = {
